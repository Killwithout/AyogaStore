...
function addProductAmount(skuId , buyLimit){
	...
	//和后台交互
	//$("#num" + skuId).val(++num);
	window.location.href = "/shopping/buyCar.shtml?skuId=" + skuId + "&amount=1&buyLimit=" + buyLimit ;
}
...
function subProductAmount(skuId){
	...
	//和后台交互
	//$("#num" + skuId).val(++num);
	window.location.href = "/shopping/buyCar.shtml?skuId=" + skuId + "&amount=-1";
}
//删除
...
<c:if test="${fn:length(buyCar.items) != 0 }">
<div class="w ofc case">
	<div class="confirm">
		...
		<div class="ofc pb40">
			...
			<tbody>
				<c:forEach items="${buyCar.items }" var="item" >
					<tr>
					<td class="nwp pic">
						<ul class="uls">
							<li>
								<a class="pic" title="${item.sku.product.name }" href="javascript:void(0)">
								<img src="${item.sku.product.img.allUrl }"></a>
								<dl>
			...
			<div class="page">
				...
				<span class="r box_gray">
					<dl class="total">
						<dt>购物车金额小计：<cite>(共<var id="productAmount">${buyCar.productAumount }</var>个商品)</cite></dt>
						<dd><em class="l">商品金额：</em>￥<var id="productPrice">${buyCar.productPrice }</var>元</dd>
						<dd><em class="l">运费：</em>￥<var id="fee">${buyCar.fee }</var>元</dd>
						<dd class="orange"><em class="l">应付总额：</em>￥<var id="totalPrice">${buyCar.totalPrice }</var>元</dd>

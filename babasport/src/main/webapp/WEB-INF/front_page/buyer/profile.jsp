...
<script type="text/javascript">
	...
	function changeProvince(code){
		...
		$.post(url,params,function(data){
			...
			var html = '<option value="" selected>城 市</option>';//
			for(var i = 0; i < cityList.length; i++){
				html += '<option value="'+ cityList[i].code +'" selected> ' + cityList[i].name + ' </option>';
			}
			$("#city").html(html);
	...
	function changeCity(code){
		...
		$.post(url,params,function(data){
			...
			for(var i = 0 ; i < townList.length; i++){
				html += '<option value="'+ townList[i].code +'" selected> ' + townList[i].name + ' </option>';
			}
			$("#town").html(html);
		},"json");
	}
		...
		<div class="confirm">
			...
			<div class="ofc">
				...
				<form id="jvForm" action="profile.shtml" method="post">
					...
					<li>
						<label for="gender">性　　别：</label><!-- ${buyer.gender.name} 枚举举例子 -->
						<span class="word">
